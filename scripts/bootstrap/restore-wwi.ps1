...
$backupFile = Require-EnvValue -Name 'WWI_BACKUP_FILENAME'
...
try {
    ...
    Invoke-Checked -Description 'WWI restore' -Command {
        & docker compose exec -T wwi-sqlserver $sqlcmd `
            ...
            -C `
            -b `
            -i /opt/project/sql/bootstrap/restore_wwi.sql `
            -v "DatabaseName=$database" "BackupPath=$backupPath"
    }
}
finally {
    Pop-Location
